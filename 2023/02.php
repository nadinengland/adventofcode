...
/**
 * Given solution for the daily puzzle.
 */
function solution(string $input): int {
    ...
    $lines_strings = str($input)->split('/\n/');
    $lines_numbers = $lines_strings->mapWithKeys(function ($line) use ($limits) {
        $id = str($line)->match('/Game (\d+)/')->value();
        $draw_strings = str($line)->after(': ')->matchAll('/[^;$]+/');

        $draws = $draw_strings->map(function ($draw) {
            $counts_by_colour = str($draw)->trim()->explode(', ')->mapWithKeys(function ($colour) {
                [$count, $key] = explode(' ', $colour);
                return [ $key => intval($count) ];
            });

            return $counts_by_colour->all();
        });
    ...
    return $lines_numbers
        ->map(function ($draws) use ($limits) {
            return $draws->every(function ($draw) use ($limits) {
                return (
                    Arr::get($draw, 'red') <= $limits['red']
                    && Arr::get($draw, 'green') <= $limits['green']
                    && Arr::get($draw, 'blue') <= $limits['blue']
                );
            });
        })
        ->filter()
...
/**
 * Input for from adventofcode.com
 */
