...
function solution(string $input): int {
    ...
    $elements = $lines
        ->map(fn ($str) => str($str)->split('//u'))
        ->flatMap(function ($row, $y) {
            return $row->map(function ($str, $x) use ($y) {
                return compact('str', 'x', 'y');
            });
        });

    $lookup = $elements
        ->filter(fn ($item) => str($item['str'])->test('/[^\d\.]/'))
        ->flatMap(fn ($item) => [
            [$item['x'] - 1, $item['y'] - 1],
            [$item['x']    , $item['y'] - 1],
            [$item['x'] + 1, $item['y'] - 1],
        ...
        ->mapWithKeys(fn ($coord) => [ implode(',', $coord) => true ]);

    $chunks = $elements
        ->chunkWhile(function ($item, $key, $map) {
            return match (true) {
                str($item['str'])->test('/\d/') => str($map->last()['str'])->test('/\d/'),
                $item['str'] !== '' => str($map->last()['str'])->test('/[^\d]/'),
                default => false,
            };
        })
        ->map(fn ($chunk) => [
            'x' => $chunk->pluck('x')->first(),
            'chunk' => $chunk,
            'str' => $str = collect($chunk)->pluck('str')->implode(''),
            'number' => str($str)->test('/^\d+$/'),
            'valid' => collect($chunk)
                ->map(fn ($item) => $item['x'] . ',' . $item['y'])
                ->some(fn ($key) => $lookup->has($key)),
        ])
        ->where('str', '!=', '')
        ->values();
...
function visualise($chunks) {
    ...
    foreach ($chunks as $item) {
        ...
        $output->write(match (true) {
            $item['number'] && $item['valid'] => '<question>' . $item['str'] . '</>',
            $item['valid'] => '<error>' . $item['str'] . '</>',
            default => $item['str'],
        });
    }
}
